...
export default class DrawerView<T: *> extends PureComponent<void, Props, void> {
  ...
  componentWillReceiveProps(nextProps: Props) {
    if (
      ...
      if (routes[index].routeName === 'SubDrawerOpen') {
  ...
  _handleDrawerOpen = () => {
    ...
    if (routes[index].routeName !== 'SubDrawerOpen') {
      this.props.navigation.navigate('SubDrawerOpen');
    }
  };
  ...
  _handleDrawerClose = () => {
    const { navigation } = this.props;
    const { routes, index } = navigation.state;
    if (routes[index].routeName !== 'SubDrawerClose') {
      this.props.navigation.navigate('SubDrawerClose');
    }
  };
  ...
  _updateScreenNavigation = (
    ...
    const navigationState = navigation.state.routes.find(
      (route: *) => route.routeName === 'SubDrawerClose'
    );
    if (
